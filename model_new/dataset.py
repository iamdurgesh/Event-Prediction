...
class EventDataset(Dataset):
    def __init__(self, file_path, event_vocab, max_len=77, excluded_events=None, debug=False):
        """
        Args:
            file_path (str): Path to the dataset CSV file.
            event_vocab (dict): Mapping of event names to unique IDs.
        ...
        excluded_ids = {event_vocab[event] for event in excluded_events} if excluded_events else set()
        ...
        for cycle, group in data.groupby("cycle"):
            ...
            if self.debug:
                print(f"Filtered sequence (excluded IDs {excluded_ids}): {filtered_sequence}")
