...
def evaluate_model(model, data_loader, config):
    ...
    with torch.no_grad():
        for inputs, targets, mask in data_loader:
            ...
            # Forward pass (LSTM does not use src_key_padding_mask)
            outputs = model(inputs)  # Shape: (batch_size, seq_len, vocab_size)
            # Debugging: Check vocabulary size alignment
            print(f"Config Vocabulary Size: {config['vocab_size']}")
            print(f"Output Last Dimension (Vocab Size): {outputs.size(-1)}")
            ...
            print(f"Masked Predictions: {masked_predictions[:5]}")
            print(f"Masked Targets: {masked_targets[:5]}")
